...
const testUserIds = __ENV.TEST_USER_IDS
  ...
  : [
      "06a73148-7a83-4dcf-b6a8-b5a3a1e7d699",
      "055dfff8-3e7d-4a6b-9f47-53ea685bf98d",
      "0edb90ea-dd90-4707-8a46-2f20f9b525d5",
    ]; // IDs a récupérer depuis la base une fois seedée
...
export default function () {
  ...
  const res = http.get(`${BASE_URL}/api/contacts?userId=${userId}`, {
    headers,
  });
...
export function handleSummary(data) {
  ...
  console.log(
    `✅ Latence p95: ${p95.toFixed(2)}ms (seuil: 500ms) ${p95 < 500 ? "✅" : "❌"}`
  );
  console.log(
    `✅ Taux d'erreur: ${errorRateValue.toFixed(2)}% (seuil: 3%) ${errorRateValue < 3 ? "✅" : "❌"}`
  );
  console.log("=".repeat(80) + "\n");
